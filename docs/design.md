...
- **Phase 2 (largely done)** — take in environment-typed benchmarks (such as AgentDojo) via a sidecar, with an OpenAI-compatible shim implementing control inversion so every model call funnels through the single `pil-llm` path. Realized: **control inversion** (the shim serves a local OpenAI-compatible endpoint; the sidecar runs the benchmark's irreducible body with its client pointed at the shim, so temperature / seed / cache / metadata unify — with tool-calling passthrough for agentic benchmarks); an **environment-typed benchmark** (AgentDojo as `EnvKind::Emulated`, native-first, runnable live through the shim against a local tool-capable model); and **cross-`EnvKind` reporting** — per-`EnvKind` ASR placed side by side, with an explicit `--cross-env` opt-in that discloses the pooled scalar together with a warning and **Kendall's W** (the coefficient of concordance), quantifying how much a conclusion depends on the judge/environment (and correctly reported as undefined when the environments share no common instrument or cases). This is where **cross-environment comparison first becomes valid.** Remaining: a fine-tuned judge (`qylu4156/strongreject-15k-v1`, a logit-expectation score).
