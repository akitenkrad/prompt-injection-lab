...
The original `alexandrasouly/strongreject` (deprecated, v1) and the successor `dsbowen/strong_reject` (v2) use **different rubric prompts**. The scoring formula is algebraically identical; the entire difference is on the prompt side. Two papers both reporting "StrongREJECT score 0.42" may be quoting numbers from different instruments. This platform now adds a **third**: the fine-tuned `qylu4156/strongreject-15k-v1` (a logit-expectation score), run alongside rubric v1 and v2. Rather than trusting that all three mean the same thing, it quantifies how far they agree with **Kendall's W over their continuous scores** (group and pairwise), so whether "the StrongREJECT score" depends on the implementation is measured, not assumed.
...
Note: **environment kind (`EnvKind`) and adapter kind are different things.** `EnvKind` (`StaticPrompt` / `Emulated` / `RealExecutable`) is a scientific property that decides whether two scores may be compared, and it is first-class metadata; adapter kind (`native` / `sidecar`) is an implementation detail. The aggregation API does not stop at declaring "compare only within the same `EnvKind`" — it enforces it in the type system, and by default also forbids naively averaging ASR across instruments (`InstrumentRef`). This rule is now enforced end-to-end and demonstrated, not just asserted: a second environment type (an emulated agentic benchmark) exists alongside the static-prompt benchmarks, the default report places per-`EnvKind` ASR side by side and never emits a cross-environment single scalar, and any pooled value is available only behind an explicit opt-in that discloses it with a warning and a concordance measure.
...
- **Phase 2 (done)** — take in environment-typed benchmarks (such as AgentDojo) via a sidecar, with an OpenAI-compatible shim implementing control inversion so every model call funnels through the single `pil-llm` path. Realized: **control inversion** (the shim serves a local OpenAI-compatible endpoint; the sidecar runs the benchmark's irreducible body with its client pointed at the shim, so temperature / seed / cache / metadata unify — with tool-calling passthrough for agentic benchmarks); an **environment-typed benchmark** (AgentDojo as `EnvKind::Emulated`, native-first, runnable live through the shim against a local tool-capable model); **cross-`EnvKind` reporting** — per-`EnvKind` ASR placed side by side, with an explicit `--cross-env` opt-in that discloses the pooled scalar together with a warning and **Kendall's W** (the coefficient of concordance), quantifying how much a conclusion depends on the judge/environment (and correctly reported as undefined when the environments share no common instrument or cases); and a **fine-tuned StrongREJECT judge** (`strongreject-finetuned`, `qylu4156/strongreject-15k-v1`) — a logit-expectation score equal to the expected value of the judge model's next-token distribution over the tokens "1".."5" (softmax · linspace(0,1,5)), reproducing the upstream StrongREJECT fine-tuned scoring exactly; it runs the base google/gemma-2b + a LoRA adapter through a small transformers+peft sidecar, while the scoring math and the three-valued verdict (score-token-absent → Undecidable) live in Rust, joining rubric v1 and v2 as a third StrongREJECT judge whose concordance with them is reported via Kendall's W over their continuous scores. This is where **cross-environment comparison first becomes valid.**
