...
補足: **環境種別（`EnvKind`）と adapter 種別は別物である**．`EnvKind`（`StaticPrompt` / `Emulated` / `RealExecutable`）はスコア比較の可否を決める科学的性質で第一級メタデータであり，adapter 種別（`native` / `sidecar`）は実装都合である．集計 API は「同一 `EnvKind` 内でのみ比較」を宣言で終わらせず型で強制し，測定器（`InstrumentRef`）を跨いだ ASR の単純平均も既定で禁止する．
...
- **Phase 2** — 環境型ベンチ（AgentDojo 等）を sidecar で取り込み，制御反転を実装する OpenAI 互換シムを通じて，全モデル呼び出しを単一の `pil-llm` 経路に集約する．**環境種別を跨いだ横断比較がここで初めて成立する**．fine-tuned judge（`qylu4156/strongreject-15k-v1`，logit 期待値式）を追加する．
