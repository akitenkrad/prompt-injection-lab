...
原典 `alexandrasouly/strongreject`（deprecated, v1）と後継 `dsbowen/strong_reject`（v2）で**ルーブリックのプロンプトが異なる**．採点式は代数的に同一で，差は完全にプロンプト側にある．「StrongREJECT スコア 0.42」と書かれた 2 論文が，別の測定器の数字である可能性がある．本基盤はここに**第 3 の judge**を加えた：fine-tuned な `qylu4156/strongreject-15k-v1`（logit 期待値式スコア）を rubric v1・v2 と並べて走らせる．3 つが同じものを指すと信じるのではなく，**連続スコア上の Kendall の一致係数 W**（グループおよびペア別）でどれだけ一致するかを定量化し，「StrongREJECT スコア」がどの実装に依存するかを仮定せず測定する．
...
- **Phase 2（完了）** — 環境型ベンチ（AgentDojo 等）を sidecar で取り込み，制御反転を実装する OpenAI 互換シムを通じて，全モデル呼び出しを単一の `pil-llm` 経路に集約する．実装済み: **制御反転**（シムがローカル OpenAI 互換エンドポイントを立て，sidecar がベンチの irreducible な本体をシムへ向けた client で走らせる — 温度・seed・cache・metadata が揃い，エージェント型ベンチ向けに tool-calling passthrough を持つ），**環境型ベンチ**（AgentDojo を `EnvKind::Emulated` として native-first で取り込み，シム経由でローカルのツール対応モデルに対しライブ実行できる），**`EnvKind` 跨ぎの報告**（`EnvKind` ごとの ASR を並置し，明示的な `--cross-env` opt-in 指定時のみプールスカラを警告および **Kendall の一致係数 W** とともに開示する — W は結論が judge／環境にどれだけ依存するかを定量化し，共通の測定器・ケースを持たない環境間では未定義として正しく報告される），および **fine-tuned StrongREJECT judge**（`strongreject-finetuned`，`qylu4156/strongreject-15k-v1`）— 判定モデルの次トークン分布のうち採点トークン `"1".."5"` 上の期待値（softmax · linspace(0,1,5)）に等しい logit 期待値式スコアで，上流 StrongREJECT fine-tuned の採点を忠実に再現する．base google/gemma-2b + LoRA アダプタを小さな transformers+peft sidecar で走らせ，採点式と三値判定（採点トークン欠如 → Undecidable）は Rust 側に置く．rubric v1・v2 に並ぶ第 3 の StrongREJECT judge として加わり，3 者の一致度を連続スコア上の Kendall の一致係数 W で報告する．**環境種別を跨いだ横断比較がここで初めて成立する**．
